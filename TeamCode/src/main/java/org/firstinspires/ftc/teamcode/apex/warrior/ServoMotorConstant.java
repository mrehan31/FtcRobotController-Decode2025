package org.firstinspires.ftc.teamcode.apex.warrior;

/**
 * This class is used to store constant values used for controlling Servo Motors
 */
public class ServoMotorConstant {

    // Servo positions (0.0 to 1.0) Rotator Assembly
    public static final double SHOOTER_ROTATOR_SERVO_DEFAULT_POS_FAR = 0.36;  // Default position FAR from goal
    public static final double SHOOTER_ROTATOR_SERVO_DEFAULT_POS_NEAR = 0.42; // Default position NEAR to goal
    public static final double SHOOTER_ROTATOR_SERVO_START_POS_NEAR = 0.40;  // Starting position NEAR to goal
    public static final double SHOOTER_ROTATOR_SERVO_END_POS_NEAR = 0.43;  // End position from NEAR to goal
    public static final double SHOOTER_ROTATOR_SERVO_START_POS_FAR = 0.35;  // Starting position FAR from goal
    public static final double SHOOTER_ROTATOR_SERVO_END_POS_FAR = 0.39;  // End position FAR from goal
//    public static final double FEEDER_SERVO_POS_0_DEG = 0.0;   // represents 0 degrees
    public static final double FEEDER_SERVO_POS_0_DEG = 0.09;   // represents 0 degrees
    public static final double FEEDER_SERVO_POS_120_DEG = 0.16; // approx 120 degrees on a 5-turn servo
    public static final double STOPPER_SERVO_POS_INITIAL_GATE = 0.55;
    public static final double STOPPER_SERVO_POS_OPEN_GATE = 0.70;
}